...
-- Version 2.0.0 - Revenue Structure Overhaul
...
CREATE TABLE IF NOT EXISTS `companies` (
    ...
    `timezone` VARCHAR(50) DEFAULT 'Australia/Darwin',
    ...
    `nayax_api_key` VARCHAR(255) DEFAULT NULL,
    `nayax_api_url` VARCHAR(255) DEFAULT 'https://lynx.nayax.com',
...
CREATE TABLE IF NOT EXISTS `subscription_plans` (
    ...
    `max_machines` INT UNSIGNED DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `customers` (
    ...
    `processing_fee` DECIMAL(10,2) DEFAULT NULL COMMENT 'Per-txn fee, NULL=use company default',
...
CREATE TABLE IF NOT EXISTS `machines` (
    ...
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_company_code` (`company_id`, `machine_code`),
    INDEX `idx_customer` (`customer_id`),
...
-- ================================================================
-- JOB/MAINTENANCE MANAGEMENT (UPDATED)
-- ================================================================
...
CREATE TABLE IF NOT EXISTS `job_notes` (
    ...
    `note` TEXT NOT NULL,
    `time_minutes` INT UNSIGNED DEFAULT 0 COMMENT 'Time spent in minutes',
...
-- ================================================================
-- REVENUE & COLLECTIONS (UPDATED - Simplified)
-- ================================================================
...
CREATE TABLE IF NOT EXISTS `revenue` (
    ...
    PRIMARY KEY (`id`),
    INDEX `idx_company_machine_date` (`company_id`, `machine_id`, `collection_date`),
...
-- ================================================================
-- COMMISSION PAYMENTS (UPDATED - Full Breakdown)
-- ================================================================
...
CREATE TABLE IF NOT EXISTS `commission_payments` (
    ...
    `gross_revenue` DECIMAL(10,2) NOT NULL COMMENT 'cash + card + prepaid',
    `processing_fees` DECIMAL(10,2) NOT NULL COMMENT 'card_transactions × fee_rate',
    `net_revenue` DECIMAL(10,2) NOT NULL COMMENT 'gross - processing_fees',
    `base_commission` DECIMAL(10,2) NOT NULL COMMENT 'net × commission_rate',
    `prepaid_usage` DECIMAL(10,2) NOT NULL COMMENT 'prepaid × company_rate',
    `parts_deduction` DECIMAL(10,2) DEFAULT 0.00 COMMENT 'parts_cost × commission_rate',
    `labour_deduction` DECIMAL(10,2) DEFAULT 0.00 COMMENT 'labour_cost × commission_rate',
    `adjustments_total` DECIMAL(10,2) DEFAULT 0.00 COMMENT 'Sum of line items',
    `commission_calculated` DECIMAL(10,2) NOT NULL COMMENT 'Before min($0) applied',
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_customer_period` (`company_id`, `customer_id`, `period_start`, `period_end`),
    INDEX `idx_status` (`status`),
    INDEX `idx_customer` (`customer_id`),
...
CREATE TABLE IF NOT EXISTS `nayax_devices` (
    ...
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_company_device` (`company_id`, `device_id`),
    INDEX `idx_machine` (`machine_id`),
...
CREATE TABLE IF NOT EXISTS `nayax_transactions` (
    ...
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_company_txn` (`company_id`, `transaction_id`),
...
INSERT INTO `roles` (`company_id`, `name`, `slug`, `description`, `is_system`, `permissions`) VALUES
(NULL, 'Super Admin', 'super_admin', 'Full system access', TRUE, '["*"]'),
(NULL, 'Company Admin', 'company_admin', 'Full company access', TRUE, '["company.*"]'),
(NULL, 'Manager', 'manager', 'Manager access', TRUE, '["machines.*","customers.*","jobs.*","revenue.*","reports.*"]'),
(NULL, 'Technician', 'technician', 'Technician access', TRUE, '["machines.view","jobs.*"]'),
(NULL, 'Collector', 'collector', 'Collector access', TRUE, '["machines.view","revenue.*"]'),
(NULL, 'Viewer', 'viewer', 'Read-only access', TRUE, '["*.view"]');

INSERT INTO `subscription_plans` (`name`, `slug`, `description`, `price_monthly`, `price_yearly`, `max_users`, `max_machines`, `max_customers`, `max_storage_mb`, `features`, `is_active`, `is_featured`, `sort_order`) VALUES
('Starter', 'starter', 'Perfect for small operators', 49.00, 490.00, 2, 10, 5, 500, '{"nayax_integration": false, "portal_access": false}', 1, 0, 1),
('Professional', 'professional', 'For growing businesses', 99.00, 990.00, 5, 50, 20, 2000, '{"nayax_integration": true, "portal_access": true}', 1, 1, 2),
('Business', 'business', 'Advanced features', 199.00, 1990.00, 15, 200, 100, 10000, '{"nayax_integration": true, "portal_access": true, "api_access": true}', 1, 0, 3),
('Enterprise', 'enterprise', 'Unlimited access', 399.00, 3990.00, NULL, NULL, NULL, NULL, '{"nayax_integration": true, "portal_access": true, "api_access": true, "priority_support": true}', 1, 0, 4);
